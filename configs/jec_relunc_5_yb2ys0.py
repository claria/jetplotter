...
def before_parsing(*args, **kwargs):
    ...
    config['objects']['central'] = { 
                                'input' : '~/dust/dijetana/ana/CMSSW_7_2_3/JEC_GEN.root?yb2ys0/h_genptavg'
                                }

    for i, source in enumerate(sources):
        for var in ['up','dn']:
            config['objects']['{0}_{1}'.format(source, var)] = {
                'input' : '~/dust/dijetana/ana/CMSSW_7_2_3/JEC_GEN.root?yb2ys0_{0}_{1}/h_genptavg'.format(source, var),
    ...
    config["data_lims"] = ["_yb2ys0_xmin_", "_yb2ys0_xmax_"]
    config["x_log"] =  True
    ...
    config["ax_texts"] = ["_cmsp_", "_yb2ys0_?_upperleft_", '_20fb_'] 

    config["output_path"] = 'jec_relunc_5_yb2ys0.png'
