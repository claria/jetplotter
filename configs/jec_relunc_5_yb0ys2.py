...
def before_parsing(*args, **kwargs):
    ...
    config['objects']['central'] = { 
                                'input' : '~/dust/dijetana/ana/CMSSW_7_2_3/JEC_GEN.root?yb0ys2/h_genptavg'
                                }

    for i, source in enumerate(sources):
        for var in ['up','dn']:
            config['objects']['{0}_{1}'.format(source, var)] = {
                'input' : '~/dust/dijetana/ana/CMSSW_7_2_3/JEC_GEN.root?yb0ys2_{0}_{1}/h_genptavg'.format(source, var),
    ...
    config["data_lims"] = ["_yb0ys2_xmin_", "_yb0ys2_xmax_"]
    config["x_log"] =  True
    ...
    config["ax_texts"] = ["_cmsp_", "_yb0ys2_?_upperleft_", '_20fb_'] 

    config["output_path"] = 'jec_relunc_5_yb0ys2.png'
