    ...
    for k, q2 in enumerate(q2s):
        ...
        config["ax_texts"] = [
                {'s': ur'$Q^2\!={0}\,\mathrm{{GeV}}^2$'.format(get_q2label(q2)), 'x': 1.0, 'y': 1.01, 'va': 'bottom', 'ha': 'right'},
                              # 's=sea quark / 4?x=0.5|y=0.25|va=bottom|ha=right|weight=bold',
                              # 's=gluon / 4?x=0.5|y=0.70|va=bottom|ha=right|weight=bold',
                              # 's=$u_{\\mathrm{val}}$ quark?x=0.8|y=0.70|va=bottom|ha=right|weight=bold',
                              # 's=$d_{\\mathrm{val}}$ quark?x=0.85|y=0.45|va=bottom|ha=right|weight=bold',
                              ] 
        print config['ax_texts']
        config["output_path"] = 'phdvortrag_pdfcomp_direct_overview_heracms_{0}.png'.format(q2)
