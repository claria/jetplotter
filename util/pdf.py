...
class PDF(object):
    ...
    def _identify_pdfset(self):
        ...
        if self._lhgrid_filename.startswith('CT1'):
            self._pdf_type = 'EV'
            self._has_var = False
    ...
    def _get_lhapdf_flavor(self, flavor, pdfset_filename):
        ...
        for member in range(0, npdfs):
            for (i, xi) in enumerate(self._xrange):
                if flavor < 7 or flavor == 21:
                    pdf[member][i] = pdfs[member].xfxQ(flavor, xi, self._q)
                elif flavor == 7:
                    #DVAL 1-(-1)
                    pdf[member][i] = pdfs[member].xfxQ(1, xi, self._q) - \
                                     pdfs[member].xfxQ(-1, xi, self._q)
                elif flavor == 8:
                    #UVAL 2-(-2)
                    pdf[member][i] = pdfs[member].xfxQ(2, xi, self._q) - \
                                     pdfs[member].xfxQ(-2, xi, self._q)
                elif flavor == 9:
                    #Light sea: xS=2(xubar + xdbar + xsbar)
                    pdf[member][i] = 2*(pdfs[member].xfxQ(-1, xi, self._q) + \
                                     pdfs[member].xfxQ(-2, xi, self._q) + \
                                     pdfs[member].xfxQ(-3, xi, self._q))
                else:
